...
Deuce can currently start Emacs and handle limited keyboard input (like C-x C-c). While the moving pieces are mainly there, many editor and window related parts are still missing, so it's far from usable yet. I aim to release a developer preview, 0.1.0, early summer, followed by an actual end-user alpha "when it's done" - probably late 2013. 0.1.0 also aims to be a point where other people can start to actively contribute.
