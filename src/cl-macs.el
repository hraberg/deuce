...
;; Deuce hack, extracted fn to limit the code size of cl-parse-loop-clause
(defun cl-parse-loop-clause-helper (word hash-types key-types loop-for-bindings loop-for-sets loop-for-steps ands var)
  (cond
	...
	(while
	    ...
	    (let ((var (or (pop loop-args) (gensym "--cl-var--"))))
		  ...
		  (setq word 'in loop-args (cons '(buffer-list) loop-args)))
              (cl-parse-loop-clause-helper word hash-types key-types loop-for-bindings
                                           loop-for-sets loop-for-steps ands var)
